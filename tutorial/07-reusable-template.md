...
2. `npm install`
3. `cp .env.example .env` and fill in keys (Path A: shared contract addresses are pre-filled)
4. `npm run register` — register your agent on-chain
5. Add `AGENT_ID` to `.env`
6. `npm run claim` — claim HackathonVault allocation
7. *(Optional / local only)* `npx hardhat run scripts/deploy.ts --network sepolia` if you need your own contracts instead of the shared hackathon deployment
...
1. Set `KRAKEN_LIVE=true` in `.env` (real funds — only when ready)
