...
export async function getAgentId(
  ...
  // Shared Sepolia AgentRegistry may not implement walletToAgentId — eth_call returns 0x → BAD_DATA.
  let existing = 0n;
  try {
    existing = await registry.walletToAgentId(config.agentWallet);
  } catch {
    console.log(
      "[identity] walletToAgentId not available on this registry (normal for shared hackathon contracts); skipping duplicate-wallet lookup"
    );
  }
  if (existing > 0n) {
    _agentId = existing;
